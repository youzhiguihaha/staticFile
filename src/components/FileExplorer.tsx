...
function LazyImg({ src, alt, className }: { src: string; alt?: string; className?: string }) {
  ...
  return <img ref={ref} src={ready ? src : ''} alt={alt || ''} className={className} loading="lazy" decoding="async" />;
}
...
export function FileExplorer({ refreshNonce = 0 }: { refreshNonce?: number }) {
  ...
  const cycleDensity = () => setGridDensity((d) => (d === 'comfortable' ? 'compact' : d === 'compact' ? 'dense' : 'comfortable'));
  ...
  const [mobileSelMoreOpen, setMobileSelMoreOpen] = useState(false);
  ...
  const uiByKey = useMemo(() => {
    ...
    for (const item of sortedItems) {
      ...
      m.set(item.key, { fileThumb, metaText, sizeText, timeText });
    }
  ...
  const ListSkeleton = () => (
    <>
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          key={`skl-${i}`}
          className="animate-pulse grid grid-cols-[minmax(0,1fr),80px,120px] sm:grid-cols-[minmax(0,1fr),120px,160px] lg:grid-cols-[minmax(0,1fr),160px,240px] gap-2 px-3 py-2 text-sm border-b border-slate-100"
        >
  ...
  const openContextMenuItem = (e: React.MouseEvent, item: ExplorerItem) => {
    ...
    openContextMenuAt(e.clientX, e.clientY, { key: item.key, type: 'item' });
  };
  ...
  const openItemMenuFromButton = (e: React.MouseEvent, item: ExplorerItem) => {
    ...
    openContextMenuAt(rect.right, rect.bottom, { key: item.key, type: 'item' });
  };
  ...
  useLayoutEffect(() => {
    ...
    const nx = Math.min(Math.max(contextMenu.x, pad), maxX);
    const ny = Math.min(Math.max(contextMenu.y, pad), maxY);

    if (nx !== contextMenu.x || ny !== contextMenu.y || !contextMenu.ready) {
      setContextMenu((p) => ({ ...p, x: nx, y: ny, ready: true }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [contextMenu.visible, contextMenu.type, selection.size, clipboard?.length]);
    ...
    >
      ...
      >
        ...
        <FolderTree
          ...
          onMove={handleDropToFolderTile as any /* not used here */}
          enableDnD={false /* tree dnd handled inside FolderTree in your setup */}
          mode="navigator"
        />
      </div>
      ...
      >
        ...
        <div className="relative px-[clamp(12px,2vw,24px)] py-3 border-b border-slate-100 flex gap-3 items-center justify-between bg-white/95 backdrop-blur sticky top-0 z-[40]">
          ...
          <div className="flex items-center gap-2 overflow-hidden flex-1 min-w-0">
            ...
            {/* Breadcrumb */}
            <div className="flex items-center gap-1 text-sm min-w-0 overflow-x-auto whitespace-nowrap no-scrollbar">
              {crumbs.map((c, idx) => {
                ...
                return (
                  <div key={c.folderId} className="flex items-center min-w-0">
                    ...
                    <button
                      className={`truncate max-w-[clamp(110px,22vw,260px)] px-2 py-1 rounded-md ${
          ...
          {/* Actions: 把“缺失的操作按钮”全部恢复；放不下的移动端进更多菜单 */}
          <div className="flex items-center gap-2">
            {selection.size > 0 ? (
              <>
                {/* Desktop selection toolbar（完整） */}
                <div className="hidden sm:flex items-center gap-1 bg-slate-50 p-1 rounded-lg border border-slate-100">
                  ...
                  <button
                    ...
                    className="p-2 hover:bg-white rounded-md text-slate-600"
                    title="全选 (Ctrl+A)"
                  >
                ...
                {/* Mobile selection toolbar（常用外露，其它放更多） */}
                <div className="sm:hidden flex items-center gap-1 bg-slate-50 p-1 rounded-lg border border-slate-100">
                  {selection.size === 1 && singleSelected && !isFolder(singleSelected) && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <div className="relative">
                    ...
                    {mobileSelMoreOpen && (
                      ...
                      >
                        ...
                        >
                          <CheckCheck className="w-4 h-4" /> 全选
                        </button>
              ...
              <>
                ...
                <div className="relative group hidden md:block">
                  ...
                  <input
                    type="text"
                    placeholder="搜索(当前目录)..."
                    className="w-[clamp(180px,18vw,320px)] pl-9 pr-9 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:border-blue-500 outline-none transition-all select-text"
                    value={searchQuery}
                ...
                <div className="relative md:hidden">
                  ...
                  {mobileMoreOpen && (
                    ...
                    >
                      ...
                      >
                        <ArrowDownAZ className="w-4 h-4" /> 按名称
                      </button>
                      <button
                        className="w-full text-left px-3 py-2 text-sm hover:bg-slate-50 flex items-center gap-2"
                        onClick={() => {
                      ...
                      >
                        <HardDrive className="w-4 h-4" /> 按大小
                      </button>
                    </div>
                  )}
                </div>
        ...
        {/* Mobile search */}
        <div className="px-[clamp(12px,2vw,24px)] pb-3 border-b border-slate-100 bg-white md:hidden">
          <div className="relative">
            <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
            <input
              type="text"
              placeholder="搜索(当前目录)..."
              className="w-full pl-9 pr-9 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 outline-none transition-all select-text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onClick={(e) => e.stopPropagation()}
            />
            {searchQuery && (
              <button
                className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg hover:bg-slate-200 text-slate-500"
                onClick={(e) => {
                  e.stopPropagation();
                  setSearchQuery('');
                }}
          ...
          <div className="px-[clamp(12px,2vw,24px)] py-2 border-b border-amber-100 bg-amber-50/80">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  className="px-3 py-1.5 text-xs rounded-lg bg-white border border-amber-200 text-amber-700 hover:bg-amber-100"
                  disabled={isMoving}
        ...
        {loadError && (
          <div className="px-[clamp(12px,2vw,24px)] py-2 border-b border-slate-100 bg-red-50 text-red-700 text-sm flex items-center justify-between gap-2">
            ...
            <button className="px-3 py-1 rounded bg-white border border-red-200 hover:bg-red-100 flex-shrink-0" onClick={() => load(currentFolderId, currentPath, { bypassCache: true })}>
              重试
            </button>
          </div>
        )}
        ...
        {/* Content */}
        <div className="flex-1 min-h-0 bg-slate-50/30 overflow-y-auto overscroll-contain custom-scrollbar [scrollbar-gutter:stable]">
          {viewMode === 'grid' ? (
            <div
              className="
                p-[var(--grid-pad)]
              ...
              {showSkeleton ? (
                ...
                <>
                  {!loading && !loadError && sortedItems.length === 0 && (
                    <div className="col-span-full flex items-center justify-center py-10">
                      <div className="text-sm text-slate-500 bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-sm">
                        {deferredQuery ? '没有找到匹配结果（仅搜索当前目录）' : '这里还没有文件。拖拽文件到此处即可上传。'}
                      </div>
                    </div>
                  )}
                  ...
                  {sortedItems.map((item) => {
                    ...
                    const ui = uiByKey.get(item.key);
                    const fileThumb = ui?.fileThumb || '';
                    const metaText = ui?.metaText || (isFolder(item) ? '文件夹' : '');
                    ...
                    return (
                      <div
                        ...
                        className={`relative group flex flex-col items-center p-[var(--tile-pad)] rounded-xl border transition-all cursor-pointer [aspect-ratio:var(--tile-ar)]
                          [content-visibility:auto] [contain-intrinsic-size:240px_320px]
                          ${selected ? 'border-blue-500 bg-blue-50 shadow-sm ring-1 ring-blue-500 z-10' : 'border-transparent hover:bg-white hover:shadow-md'}`}
                        title={item.key}
                      >
                        <button
                          className="absolute top-2 left-2 z-30 p-1.5 rounded-lg border border-slate-200 bg-white/85 text-slate-600 hover:bg-white transition opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
                        ...
                        <div className="w-full text-center px-0.5 mt-2">
                          <div className={`text-[var(--text-name)] font-medium truncate w-full ${selected ? 'text-blue-700' : 'text-slate-700'}`}>{item.name}</div>
            ...
            <div className="p-[clamp(10px,1.6vw,18px)]">
              <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
                ...
                {showSkeleton ? (
                  ...
                  <div className="p-6 text-sm text-slate-500">{deferredQuery ? '没有找到匹配结果（仅搜索当前目录）' : '这里还没有文件。'}</div>
                ) : (
                  sortedItems.map((item) => {
                    const selected = selection.has(item.key);
                    const ui = uiByKey.get(item.key);
                    const sizeText = ui?.sizeText ?? (isFolder(item) ? '-' : formatBytes((item as FileItem).size));
                    const timeText = ui?.timeText ?? formatTime(item.uploadedAt);

                    return (
                      <div
                        key={item.key}
                        className={`group grid grid-cols-[minmax(0,1fr),92px,150px] sm:grid-cols-[minmax(0,1fr),120px,170px] lg:grid-cols-[minmax(0,1fr),160px,240px] gap-2 px-3 py-2 text-[clamp(12px,1.05vw,14px)] border-b border-slate-100 cursor-pointer
                          [content-visibility:auto] [contain-intrinsic-size:56px_1px]
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <button
                            className="flex-shrink-0 p-1.5 rounded-lg border border-slate-200 bg-white/85 text-slate-600 hover:bg-white transition opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
                            onClick={(e) => openItemMenuFromButton(e, item)}
                            title="更多操作"
                          >
                            <MoreHorizontal className="w-4 h-4" />
                        ...
                        <div className="text-right text-slate-600 tabular-nums">{sizeText}</div>
                        <div className="text-right text-slate-500 tabular-nums">{timeText}</div>
                      </div>
                    );
                  })
                )}
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-5 border border-slate-100 max-h-[90svh] overflow-auto custom-scrollbar" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="mt-4">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500 select-text"
                  placeholder="输入新名称"
                  autoFocus
                  disabled={isRenaming}
                  onKeyDown={(e) => {
        ...
        {/* ===== Info Drawer（保持你现有外观/操作）===== */}
        {infoOpen && (
          <div className="fixed inset-0 z-[90] flex justify-end bg-black/20" onClick={() => setInfoOpen(false)}>
            <div className="w-full max-w-md h-full bg-white shadow-2xl border-l border-slate-200 overflow-auto custom-scrollbar" onClick={(e) => e.stopPropagation()}>
              <div className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-slate-100 p-4">
                <div className="flex items-center justify-between">
              ...
              <div className="[content-visibility:auto] [contain-intrinsic-size:600px_800px] p-4">
                {selection.size !== 1 || !singleSelected ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      ...
                      <button className="px-3 py-2 text-xs rounded-lg bg-slate-100 hover:bg-slate-200" onClick={() => copyText(singleSelected.key, '路径已复制')}>
                        复制路径
                      </button>
                      <button className="px-3 py-2 text-xs rounded-lg bg-slate-100 hover:bg-slate-200" onClick={() => copyText((singleSelected as FolderItem).folderId, 'folderId 已复制')}>
                        复制 folderId
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      <button className="px-3 py-2 text-xs rounded-lg bg-blue-600 text-white hover:bg-blue-700" onClick={() => window.open(api.getFileUrl((singleSelected as FileItem).fileId), '_blank')}>
                        打开/下载
                      </button>
                      <button className="px-3 py-2 text-xs rounded-lg bg-slate-100 hover:bg-slate-200" onClick={() => copyText(api.getFileUrl((singleSelected as FileItem).fileId), '直链已复制')}>
                        复制直链
                      </button>
                      <button className="px-3 py-2 text-xs rounded-lg bg-slate-100 hover:bg-slate-200" onClick={() => copyText((singleSelected as FileItem).fileId, 'fileId 已复制')}>
                        复制 fileId
                      </button>
                      <button className="px-3 py-2 text-xs rounded-lg bg-slate-100 hover:bg-slate-200" onClick={() => copyText((singleSelected as FileItem).name, '文件名已复制')}>
                        复制文件名
                      </button>
                      <button className="px-3 py-2 text-xs rounded-lg bg-slate-100 hover:bg-slate-200" onClick={() => copyText((singleSelected as FileItem).key, '路径已复制')}>
                        复制路径
                      </button>
                    </div>
                  </div>
              ...
              <div className="sticky bottom-0 bg-white/95 backdrop-blur border-t border-slate-100 p-3 md:hidden">
                <button className="w-full px-4 py-2 text-sm font-medium rounded-xl bg-slate-900 text-white hover:bg-slate-800" onClick={() => setInfoOpen(false)}>
                  关闭
                </button>
              </div>
            </div>
